...
st.markdown("""
    ...
    .hero-title { font-size: 56px; font-weight: 800; color: #1D1D1F; text-align: center; margin-top: 60px; letter-spacing: -2px; line-height: 1.1; }
    .hero-subtitle { font-size: 24px; color: #86868B; text-align: center; margin-bottom: 60px; font-weight: 400; }
    ...
    div[data-testid="stButton"] button:hover { background-color: #0077ED !important; transform: scale(1.02); }

    .metric-container { text-align: center; padding: 25px; background: white; border-radius: 24px; border: 1px solid #F2F2F7; }
    ...
    .prd-item { display: flex; justify-content: space-between; padding: 16px 0; border-bottom: 1px solid #F5F5F7; }
    .prd-item b { color: #1D1D1F; font-weight: 600; }
    .prd-item span { color: #48484A; }

    .section-header { color: #1D1D1F; font-size: 24px; font-weight: 700; margin: 40px 0 20px 0; border-left: 5px solid #0071E3; padding-left: 15px; }
    </style>
...
if st.session_state.step == "概念导入":
    ...
    st.markdown("""
        <div class="st-card" style="text-align: center;">
            <p style="color: #48484A; font-size: 17px; max-width: 850px; margin: 0 auto; line-height: 1.6;">
                在选购具身智能终端前，通过科学的算法逻辑，对比您的隐私安全阈值、家庭结构及核心情感诉求，从企业视角为您精准匹配最合适的硬件形态。
            </p>
        </div>
    """, unsafe_allow_html=True)
    ...
    with p14: display_product_image("圆宝陪伴机器人.jpg.jpg", "圆宝机器人")
...
# --- 结论页 ---
    ...
    with col_rec_l:
        if persona == "高知尝鲜派": display_product_image("Ameca.jpg", "旗舰推荐：Ameca")
        elif persona == "亲子伴学派": display_product_image("ASUS Zenbo.jpg", "推荐方案：ASUS Zenbo")
        elif persona == "普惠务实派": display_product_image("loona.jpg", "推荐形态：Loona 智能宠")
        else: display_product_image("Aeolus Bot.jpg", "推荐形态：Aeolus Bot")
    with col_rec_r:
        st.markdown(f"""<div class="st-card"><h4 style="color:#1D1D1F; margin-top:0;">📋 企业级规格配置单 (PRD)</h4>
        <div class="prd-item"><b>核心卖点</b> <span>解决“{resp['func']}”的情感/功能需求</span></div>
        ...
        <p style="font-size: 13px; color: #86868B; margin-top: 25px;">* 本建议基于成都市具身智能市场研究 XGBoost 权重模型动态生成。</p></div>""", unsafe_allow_html=True)
...
# --- 场景演练页 ---
    ...
    if persona == "高知尝鲜派":
        ...
        <div class="st-card" style="border-left: 5px solid #0071E3;">
            <h4>⏰ 08:00 | 硅基唤醒：晨间深度简报</h4><p>机器人同步您的睡眠数据，识别唤醒周期后通过温和语音播报当日全球科技要闻及您的深度行程建议。</p>
            <h4>⏰ 13:00 | 商务辅助：异地会议代理</h4><p>在您办公时，机器人通过大模型辅助处理纪要，并利用精准微表情替代您进行远程商务对谈。</p>
            <h4>⏰ 19:00 | 归家迎接：情绪实时感知</h4><p>识别到下班后的疲惫特征，主动发起“树洞对话”，提供高情商心理疏导，并联动全屋调至解压模式。</p>
            <h4>⏰ 23:30 | 社交代偿：深夜灵魂伴侣</h4><p>在寂静深夜提供高层次的语义逻辑碰撞，成为完美的情感容器，有效缓解都市孤独感。</p>
        </div>""", unsafe_allow_html=True)
    elif persona == "亲子伴学派":
        st.markdown("""
        <div class="st-card" style="border-left: 5px solid #0071E3;">
            <h4>⏰ 07:30 | 智慧晨起：今日学习路线</h4><p>通过拟人化互动叫醒孩子，同步播报今日学习计划，激发孩子学习自驱力。</p>
            <h4>⏰ 15:30 | 安全防线：离线全屋监测</h4><p>设备进入最高隐私安全模式。一旦识别孩子靠近厨房或洗手间高危区，即刻全屋警报并连线家长。</p>
            <h4>⏰ 19:00 | 知识百科：沉浸式绘本伴读</h4><p>利用本地算力精准识别各类教材，用多变声线陪孩子互动问答，实现高质量教育陪伴。</p>
            <h4>⏰ 21:00 | 助眠仪式：AI 故事创作</h4><p>现场生成专属的互动童话故事，辅助孩子在安稳氛围中入睡。</p>
        </div>""", unsafe_allow_html=True)
    elif persona == "普惠务实派":
        st.markdown("""
        <div class="st-card" style="border-left: 5px solid #86868B;">
            <h4>⏰ 09:30 | 静谧陪伴：办公氛围营造</h4><p>无需指令，仿生宠物在脚边安静蜷缩，模拟真实心跳，为您营造安心的居家办公环境。</p>
            <h4>⏰ 12:30 | 轻度解压：午间治愈互动</h4><p>简单的逗趣互动即可触发拟真反馈，通过低成本交互切断工作内耗，提供瞬时情绪补位。</p>
            <h4>⏰ 19:30 | 物理抚慰：触觉反馈治愈</h4><p>您坐在沙发休息，抚摸设备会触发模拟“呼噜声”。这种高亲和力的物理反馈，能有效降低焦虑。</p>
            <h4>⏰ 22:30 | 稳定存在：非侵入式守候</h4><p>设备反馈以柔和的机械微动，在寂静深夜提供一份“生命共处感”。</p>
        </div>""", unsafe_allow_html=True)
    else: # 传统保守派
        st.markdown("""
        <div class="st-card" style="border-left: 5px solid #0071E3;">
            <h4>⏰ 08:30 | 健康扫描：晨间体征监测</h4><p>雷达扫描老人步态。准时语音提醒服用降压药，确认服药后自动向子女发送报告。</p>
            <h4>⏰ 11:00 | 社交连接：跨代际视频助手</h4><p>通过最简化的声控交互开启远程通话，协助老人与异地子女沟通。</p>
            <h4>⏰ 14:00 | 生命兜底：跌倒预警防线</h4><p>全天候扫描全屋。若识别到洗手间区域高度突变且30秒未移动，系统在尊严保护下直连子女。</p>
            <h4>⏰ 21:00 | 隐形守护：居家安全巡视</h4><p>利用声学与雷达确认居家安全隐患。在长者入睡后维持后台监测，实现真正的无感化“平安哨兵”。</p>
        </div>""", unsafe_allow_html=True)
